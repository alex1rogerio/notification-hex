...
public class NotificationPersistencePortImpl implements NotificationPersistencePort {
	...
	public Optional<NotificationDomain> findByNotificationIdAndUserId(UUID notificationId, UUID userId) {
			...
			Optional<NotificationEntity> notificationEntityOptinal = notificationJpaRepository.findByNotificationIdAndUserId(notificationId, userId);
			if (notificationEntityOptinal.isPresent()) {
				return Optional.of(modelMapper.map(notificationEntityOptinal.get() , NotificationDomain.class));
			}else {
